...
usgin.CSW.prototype.tagServiceType = function(rcd) {
	for(var i=0; i<rcd.references.length; i++) {
		var rf = usgin.Util.validVar(rcd.references[i]) ? rcd.references[i].toLowerCase() : "";
		if ( rf.search('request=getcapabilities') >0 && rf.search('service=wms') >0 ) {
			rcd.serviceTypes.push("WMS");
		}	
		if ( rf.search('request=getcapabilities') >0 && rf.search('service=WFS') >0 ) {
			rcd.serviceTypes.push("WFS");
		}	
	}	
}
