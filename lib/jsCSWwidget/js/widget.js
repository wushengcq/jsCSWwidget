...
usgin.CSWwidget = function(container, conf) {
	...
	this.spinner = null;
	this.infoBoard = null; this.infoBoardWidth = -1;
	...
	this.recordSelectedHandler = null;
	this.recordChosenListener = null;
...
usgin.CSWwidget.prototype.init = function() {
	...
	var frm = $("<table><tr><th class='title' colspan=3></th></tr></table>").addClass('csw-table');	
	...
	frm.append($("<tr>")
		...
		.append(this.getLaunchBtn()).append(this.getSpatialBtn()).append(this.getSubjectBtn()).append(this.getAbstractBtn()))
	).append($("<tr>")
...
usgin.CSWwidget.prototype.getLaunchBtn = function() {
	if (this.launchBtn == null) {
		var btn = $($.parseHTML("<i class='fa fa-plus fa-fw csw-infoboard-btn csw-infoboard-btn-disabled' title='add layer'/>"));
		var w = this;
		btn.click(function(e) {
			if (usgin.Util.isFunction(w.recordChosenHandler)) {
				w.wmswfs(w.recordChosenHandler);
            }
		});
		this.launchBtn = btn;
	}
...
usgin.CSWwidget.prototype.showInfo = function(theme) {
	...
	if (this.infoBoardWidth < 0) {
		this.infoBoardWidth = $('.csw-infoboard-box').width();
	}
	this.getInfoBoard().width(this.infoBoardWidth-8);
	...
	if (usgin.Util.validVar(opt) && opt.length > 0) {
		...
		if (usgin.Util.validVar(rc)) {
			...
			this.getInfoBoard().remove
			this.getInfoBoard().removeClass(function(idx, css){
				var ss = css.split(" "), rslt = "";
				for (var i=0; i<ss.length; i++) {
					rslt += (ss[i].indexOf("csw-infoboard-theme-") >= 0) ? " "+ss[i] : "";
				}
				return rslt;
			}).addClass("csw-infoboard-theme-" + theme);
			...
			if (usgin.Util.isFunction(this.recordSelectedHandler)) {
				this.wmswfs(this.recordSelectedHandler);
			}
		}
	}else{
		this.clearInfo();
...
usgin.CSWwidget.prototype.wmswfs = function(callback) {
	var opt = this.getRecordSelector().find("option:selected");
	if (usgin.Util.validVar(opt) && opt.length > 0) {
		var rc = this.getRecordById(opt.val());
		var layers = [];
		try {
			this.wmswfsChain(rc, 0, layers, callback);
		} catch(e) {
			jsn = {"result":{
				"status": "ERROR",
				"message": e 
...
usgin.CSWwidget.prototype.wmswfsChain = function(rc, idx, layers, func) {
	var refs = rc.references;
	if (idx >= refs.length) {
		// when get all the layers informatioin of wfs and wms in 'references' array recursively, 
		// return the result and end the recursive funcion call.
		var jsn = null;
		if (layers.length > 0) {
			jsn = {"result":{
				"status": "OK",
	...
	if ( url.search('request=getcapabilities') >0 && url.search('service=wms') >0 ) {
		...
		wms.getLayers (function(lys) {
			...
			w.wmswfsChain(rc, idx+1, layers, func);
		}, function(msg){
			throw msg;
		});
...
usgin.CSWwidget.prototype.addRecordSelectedHandler = function(lsnr) {
	this.recordSelectedHandler = lsnr;
}

usgin.CSWwidget.prototype.addRecordChosenHandler = function(hdr) {
	this.recordChosenHandler = hdr;
}
